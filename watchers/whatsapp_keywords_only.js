    ...
    while (true) {
        ...
        for (const chatItem of chatListRows) {
            ...
            if (unreadIndicator) {
                ...
                // Wait for messages to load properly
                await page.waitForTimeout(2000); // Give extra time for messages to load

                // Get full chat content with multiple fallback strategies
                let fullChatContent = '';
                
                // Strategy 1: Get all message text elements with proper waits
                try {
                    // Wait for message elements to appear
                    await page.waitForSelector('span.selectable-text.copyable-text span, [data-testid="conversation-panel-messages"] span', { timeout: 5000 });
                        ...
                        try {
                            ...
                            if (messageElements.length > 0) {
                                for (const element of messageElements) {
                                    ...
                                    if (text && text.trim() !== '') {
                                        // Skip timestamps, contact names, and metadata
                                        if (!text.includes('@') && 
                                            !text.match(/^\d{1,2}[:]\d{2}/) && 
                                            !text.match(/online|now|yesterday/) && 
                                            text.length > 2) {
                    ...
                    try {
                        ...
                        if (mainElement) {
                            ...
                            const lines = mainText.split('\n').filter(line => 
                                ...
                                !line.match(/online|now|yesterday|search|type|message/i) &&
                                line.length > 5
                            );
                            fullChatContent = lines.join(' ').substring(0, 2000);
                        }
                    } catch (err) {
                ...
                if (fullChatContent.length > 2000) {
                    ...
                    fullChatContent = lines.slice(-15).join(' ').substring(0, 2000);
                }

                console.log(`[DEBUG] Full chat content length: ${fullChatContent.length}`);
                
                // Only proceed if we have meaningful content to check
                if (fullChatContent.length > 10) {
...
`;
                    ...
                    console.log(`[WhatsApp Watcher] Insufficient content to check in chat "${chatTitle}". Skipping.`);
                }
